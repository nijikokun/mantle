...
const selectTriggerVariants = cva(
	"flex h-11 w-full items-center justify-between rounded-md border border-form bg-form px-3 py-2 placeholder:text-placeholder hover:bg-form-hover focus:outline-none focus:ring-4 disabled:pointer-events-none disabled:opacity-50 aria-expanded:ring-4 sm:h-9 sm:text-sm [&>span]:line-clamp-1 [&>span]:text-left",
	{
		variants: {
			state: {
				danger:
					"border-red-600 focus:border-red-600 focus:ring-danger aria-expanded:border-red-600 aria-expanded:ring-danger",
				default:
					"text-strong borderpform placeholder:text-placeholder focus:border-blue-600 focus:ring-accent aria-expanded:border-blue-600 aria-expanded:ring-accent",
			},
		},
		defaultVariants: {
			state: "default",
...
>(({ className, children, ...props }, ref) => (
	<SelectPrimitive.Item
		ref={ref}
		className={cx(
			"relative flex w-full cursor-pointer select-none items-center rounded py-1.5 pl-2 pr-8 text-sm outline-none focus:bg-popover-hover data-state-checked:bg-blue-500 data-state-checked:text-inverted data-disabled:pointer-events-none data-disabled:opacity-50",
			className,
		)}
		{...props}
	>
